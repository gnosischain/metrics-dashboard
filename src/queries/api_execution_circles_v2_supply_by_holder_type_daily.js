const metric = {
  ...
  metricDescription: `Daily CRC supply split by the category of the address holding it. Each holder is labelled by its Circles avatar type (\`Human\`, \`Group\`, \`Org\`) when it is a registered avatar; otherwise by a crawler/Dune-style label \`project\` (e.g. \`DEX\`, \`Wallets and AA\`); otherwise \`Other\`. Values sum positive balances only — the zero address and any zero/negative balances are excluded — in CRC. Toggle **Static** (nominal \`supply\`) vs **Demurraged** (demurrage-adjusted \`demurraged_supply\`); the current incomplete day is excluded.`,
  chartType: 'area',
  isTimeSeries: true,
  stacked: true,
  enableZoom: true,
