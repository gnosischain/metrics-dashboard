...
const metric = {
  ...
  metricDescription: `Every **liquidity event** for the selected pool, most recent first. \`Type\` is **Add** (Uniswap V3 \`Mint\` — an LP deposits liquidity) or **Remove** (\`Burn\` — position principal withdrawn). The two token columns show the amount of **each reserve token added or removed** by that event (position principal; separately-collected fees via \`Collect\` are not shown). \`Value\` is the event's USD size — each leg valued at its as-of price (CRC legs at the trade VWAP, stablecoin legs at the oracle); an event that predates the token's first market price, such as the pool-creation mint, is valued at that first observed price. \`LP\` is the position owner recorded in the event and \`Tx\` links to the transaction on Blockscout. Uniswap V3 pools only (the Balancer pool renders empty here).`,
  chartType: 'table',
