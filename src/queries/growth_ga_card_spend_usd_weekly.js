const metric = {
  ...
  metricDescription: `Weekly USD **card payments** (\`action='Payment'\`, actual merchant spend) on Gnosis-App-controlled Gnosis Pay cards, split by \`onboarding_class\` (\`onboarded_via_ga\` = the GP Safe's first owner was a GA user; \`imported\` = a GA owner joined an existing GP Safe later). Scoped to GA-controlled wallets and counted only from \`first_ga_owner_at\` onward, so this is the GA slice, NOT the all-Gnosis-Pay total (which lives on the gnosis-pay dashboard). Unit: USD; the current incomplete week is excluded.`,
  "chartType": "bar",
  "isTimeSeries": true,
  "enableZoom": true,
  "xField": "date",
