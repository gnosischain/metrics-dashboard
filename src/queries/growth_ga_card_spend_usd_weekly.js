const metric = {
  ...
  metricDescription: `Weekly USD **card payments** (\`action='Payment'\`, actual merchant spend) on Gnosis-App-controlled Gnosis Pay cards, split by **link source** (how the card is known to be GA-linked): app profile (Mixpanel), Delay module (legacy), top-up funder, cashback. Sourced from the module-agnostic all-architectures GA-linked card set with activity **canonicalized** (old→new Safe) and counted from each card's GA-control-start (\`coalesce(first_ga_owner_at, first activity)\`), so it no longer declines post-migration. This is the GA slice, NOT the all-Gnosis-Pay total (which lives on the gnosis-pay dashboard). Unit: USD; the current incomplete week is excluded.`,
  "chartType": "bar",
  "isTimeSeries": true,
  "enableZoom": true,
  "showTotal": true,
