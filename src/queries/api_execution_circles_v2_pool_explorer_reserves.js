const metric = {
  ...
  metricDescription: `Current reserves of **both tokens** in the selected pool — one row per leg. \`Reserve\` is the token amount held by the pool (Uniswap V3: reconstructed from event deltas, matches on-chain \`balanceOf\` within a few %; Balancer: from the vault balances). \`Price\` is the token's USD price (CRC legs from their trade VWAP, stablecoin legs from the oracle) and \`Value\` = reserve × price — the two Values sum to the pool's TVL.`,
  chartType: 'table',
