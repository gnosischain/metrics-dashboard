const metric = {
  ...
  metricDescription: `Running cumulative total of **Gnosis App marketplace buys** per offer, plotted over a continuous daily spine (empty days back-filled so each offer's line stays monotonic). A **buy** is a \`PaymentReceived\` event on a Circles \`PaymentGateway\` relayed by an active Cometh ERC-4337 bundler where the \`payer\` is a known Gnosis App user; an **offer** is a named gateway from \`createGateway\`, minus those on the exclusion list. Each series is that offer's \`cumulative_buys\` through the date; the current in-progress day is excluded. Unit: cumulative buys (count).`,
  chartType: 'area',
  isTimeSeries: true,
  stacked: true,
  showTotal: true,
