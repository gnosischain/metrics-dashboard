...
const extractField = (content, field) => {
  ...
  const value = match[2]
    ...
    .replace(/\\n/g, '\n')
    // Do not remove — sources are checked out CRLF wherever git's core.autocrlf is on, so
    // without this the registry payload differs between a Windows and a Linux build. Nothing
    // in CI catches its removal. See docs/lessons/crlf-export-drift.md
    .replace(/\r\n?/g, '\n')
    .trim();
  return value || null;
};
